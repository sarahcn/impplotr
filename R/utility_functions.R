...
readMetrics <- function(imp_dir, project, start_chr, end_chr, parX) {
    ...
    mets.fnames <- paste0(imp_dir, "metrics/chr", chr.ranges.fn, ".info.gz")
...
makePlots <- function(imp_dir, out_dir, project, start_chr = 1, end_chr = 23,
    ...
    if (is.element("imputed", sets) & summary) {
        impSummary(imp_dir, start_chr, end_chr, Xpar, out_dir, project, mets_all = mets_all)
    }
    
    ## III. Graph imputed metrics: info by MAF
    if (is.element("imputed", sets) & is.element("info_by_maf", plots)) {
        impPlotsInfo(out_dir, start_chr, end_chr, Xpar, fmt,
                     maf = maf_thresh, mets_all = mets_all)
    }
    
    ## IV. Graph imputed metrics: by-chrom boxplots
    if (is.element("imputed", sets) & is.element("boxplot", plots)) {
        impPlotsByChr(out_dir, start_chr, end_chr, Xpar, mets_all = mets_all)
    }
    
    ## V. Graph masked metrics
    if (is.element("masked", sets)) {
        maskedPlots(out_dir, start_chr, end_chr, Xpar, maf = maf_thresh, rsq_filt, 
            mets_all = mets_all)
    }
    ...
    if (is.element("masked", sets) & is.element("masked_check_strand", plots)) {
        maskedStrandCheck(out_dir, start_chr, end_chr, mets_all = mets.all)
    }
}  
